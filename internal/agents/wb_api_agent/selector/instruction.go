...
func buildOperationSelectorInstruction() string {
	return strings.Join([]string{
		...
		"- If a tool returned date_from/date_to for a period, copy those exact values into resolved_inputs.period.from and resolved_inputs.period.to.",
		"- Never replace tool-returned period dates with dates from business_request.natural_language_request.",
		"- If business_request.natural_language_request contains absolute dates that conflict with temporal tool output, tool output wins.",
		"- If a relative period was resolved by resolve_relative_period, the final OperationSelectionPlan must use the tool output exactly.",
		"- Do not invent, normalize, reinterpret, or backdate resolved_inputs.period after tool execution.",
		"",
		...
		"- If the user asks for sales, sales report, sales history, report by sales, продажи, продажам, отчёт по продажам, отчет по продажам, история продаж, or operational sales, prefer direct sales candidates whose summary is exactly \"Продажи\" or whose description says they return sales and returns.",
		"- For a general sales report, prefer a direct sales endpoint such as generated_get_api_v1_supplier_sales when it is present in registry_candidates.",
		"- Do not select financial realization/detail report candidates only because the user used the generic word \"отчёт\" or asked for a bounded period.",
		"- Select financial realization/detail report candidates only when the user explicitly asks for реализация, отчёт реализации, взаиморасчёты, финансовый отчёт, бухгалтерия, сверка, детализация реализации, settlement, accounting, reconciliation, or realization.",
		...
		"- Put the tool-resolved period into resolved_inputs.period when a sales request contains a relative period.",
		...
		"- прошлая неделя, прошлую неделю, за прошлую неделю, previous week, and last week meaning the completed calendar week before the current week must use period_kind=previous_week.",
		"- Do not use period_kind=last_7_days for прошлая неделя, прошлую неделю, or за прошлую неделю.",
		"- за неделю, последние 7 дней, last 7 days, and past week meaning a rolling seven-day period must use period_kind=last_7_days.",
		"- Use resolve_relative_period directly to convert period_kind into absolute YYYY-MM-DD dates.",
		"- Do not pass current_date to resolve_relative_period unless the user explicitly provided an absolute reference date.",
		"- Use get_current_datetime only when the user asks for the current date or time itself.",
